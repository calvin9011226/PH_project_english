...
#全部
log = Log(Auto_Clear=True,File_Only=True,File_Name="all/all_log",choose=["no"])
timer = CodeTimer()
#行程規劃
log2 = Log(Auto_Clear=True,File_Only=True,File_Name="focus/focus_log",choose=["no"])
timer2 = CodeTimer()

#臨時用
log3 = Log(Auto_Clear=True,File_Only=True,File_Name="focus/focus_location_log",choose=["no"])
timer3 = CodeTimer()
...
def update_plan_csv_with_populartimes(plan_csv_file,option):
    ...
    timer_stop_log(tag="景點排序",group=option, content=plan_df, timer=timer2, log=log2)
...
def process_travel_planning(option, reply_token, user_id):
    ...
    timer2.start(tag="機器學習",group=option)
    try:
        ...
        return
    timer_stop_log(tag="機器學習",group=option, content=plan_data, timer=timer2, log=log2)

    timer2.start(tag="塞選景點",group=option)
    try:
        ...
        timer_stop_log(tag="塞選景點",group=option ,content=plan_content, timer=timer2, log=log2)
        
        timer2.start(tag="景點排序",group=option)
        update_plan_csv_with_populartimes(csv_plan,option)
        
        timer2.start(tag="寫入資料庫",group=option)
        Plan2MYSQL.plan3mysql(csv_plan)
        timer_stop_log(tag="寫入資料庫",group=option , timer=timer2, log=log2)
...
def generate_timeline():
    ...
    timer3.generate_timeline_plot(file_name="focus/timeline_2",size_off=True)
    return "✅ 時序圖已建立"
